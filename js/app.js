...
async function loadCycleSettings() {
  ...
  if (!targetUserId) {
    ...
    cycleData.settings = { cycleLength: 28, periodLength: 5, periodOverrides: {} };
    renderPhaseCard();
    renderAdvice();
    renderCalendar();
    return;
  ...
  if (settings) {
    if (!settings.periodOverrides) settings.periodOverrides = {};
    cycleData = calculateCycle(settings);
  ...
  renderPhaseCard();
  renderAdvice();
  // Календарь сразу при загрузке
  renderCalendar();
}

async function saveCycleSettings(lastPeriodStart, cycleLength, periodLength, periodOverrides) {
  const prev = cycleData?.settings || {};
  const data = {
    lastPeriodStart,
    cycleLength: cycleLength || prev.cycleLength || 28,
    periodLength: periodLength || prev.periodLength || 5,
    periodOverrides: periodOverrides !== undefined ? periodOverrides : (prev.periodOverrides || {}),
    updatedAt: new Date().toISOString()
  };
...
async function persistSettings(settings) {
  ...
  if (window.USE_LOCAL_FALLBACK || !window.db) {
    localStorage.setItem(`cycle_${targetUserId}`, JSON.stringify(settings));
  } else {
    await window.firebaseModules.setDoc(window.firebaseModules.doc(window.db, 'cycles', targetUserId), settings, { merge: true });
  }
  cycleData = calculateCycle(settings);
  cycleData.settings = settings;
  renderPhaseCard();
...
function openLogModal(dateStr) {
  ...
  window._selectedDay = selectedLogDate;
...
function renderCalendar() {
  ...
  const days = getCalendarDays(calendarYear, calendarMonth, settings);
  ...
  const html = ['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'].map(d => `<div class="cal-day-name">${d}</div>`).join('') +
    days.map(d => {
      ...
      else if (d.isFollicular) cls += ' follicular';
      else if (d.isLuteal) cls += ' luteal';
      if (logDates.has(dateStr)) cls += ' has-log';
      if (d.override === true) cls += ' period-manual';
      if (d.override === false) cls += ' period-cleared';
      return `<button class="${cls}" onclick="onDayClick('${dateStr}')" title="${dateStr}">${d.day}</button>`;
    }).join('');
...
function onDayClick(dateStr) {
  openDayActions(dateStr);
}

function openDayActions(dateStr) {
  selectedLogDate = dateStr;
  window._selectedDay = dateStr;
  const settings = cycleData?.settings || {};
  const info = settings.lastPeriodStart ? getDayInfo(dateStr, settings) : {};
  const isPeriod = !!info.isPeriod;

  document.getElementById('dayActionDate').textContent = formatDate(dateStr);
  document.getElementById('dayActionPeriodLabel').textContent = isPeriod
    ? '🩸 Сейчас отмечено как месячные'
    : 'Не месячные';
  document.getElementById('btnTogglePeriod').textContent = isPeriod
    ? '❌ Убрать день месячных'
    : '🩸 Отметить как день месячных';
  document.getElementById('dayActionsModal').classList.add('open');
}

async function togglePeriodForSelected() {
  if (!selectedLogDate) return;
  const settings = cycleData?.settings || { cycleLength: 28, periodLength: 5, periodOverrides: {} };
  if (!settings.lastPeriodStart) {
    // Первая отметка — ставим как начало
    settings.lastPeriodStart = selectedLogDate;
    settings.periodOverrides = settings.periodOverrides || {};
    settings.periodOverrides[selectedLogDate] = true;
    await persistSettings(settings);
    closeModal('dayActionsModal');
    showToast('Начало месячных установлено');
    return;
  }
  const updated = togglePeriodDay(settings, selectedLogDate);
  await persistSettings(updated);
  closeModal('dayActionsModal');
  showToast('День месячных обновлён');
}

function openPeriodRangeModal() {
  closeModal('dayActionsModal');
  const s = cycleData?.settings || {};
  document.getElementById('periodRangeStart').value = selectedLogDate || s.lastPeriodStart || '';
  document.getElementById('periodRangeEnd').value = selectedLogDate || '';
  document.getElementById('periodRangeModal').classList.add('open');
}

async function savePeriodRange(e) {
  e.preventDefault();
  const start = document.getElementById('periodRangeStart').value;
  const end = document.getElementById('periodRangeEnd').value;
  if (!start || !end) {
    showToast('Укажи начало и конец');
    return;
  }
  if (end < start) {
    showToast('Конец не может быть раньше начала');
    return;
  }
  const settings = cycleData?.settings || { cycleLength: 28, periodLength: 5, periodOverrides: {} };
  const updated = setPeriodRange(settings, start, end);
  await persistSettings(updated);
  closeModal('periodRangeModal');
  showToast('Период месячных обновлён 💜');
}
...
window.openDayActions = openDayActions;
window.togglePeriodForSelected = togglePeriodForSelected;
window.openPeriodRangeModal = openPeriodRangeModal;
window.savePeriodRange = savePeriodRange;
